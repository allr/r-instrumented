...
/* forward declaration */
static SEXP allocVector3Internal(SEXPTYPE type, R_xlen_t length,
                                 R_allocator_t *allocator, Rboolean count_allocation);
...
unsigned long allocated_list, allocated_list_elts;
...
char *R_alloc(size_t nelem, int eltsize)
{
	...
	s = allocVector3Internal(RAWSXP, size + 1, NULL, FALSE);
#else
...
SEXP allocVector3(SEXPTYPE type, R_xlen_t length, R_allocator_t *allocator) {
    return allocVector3Internal(type, length, NULL, TRUE);
}
                                 ...
                                 R_allocator_t *allocator, Rboolean count_allocation)
{
    ...
    int node_class;
#if VALGRIND_LEVEL > 0 || defined(TRACER_COMPILED_IN)
...
#if VALGRIND_LEVEL > 1 || defined(TRACER_COMPILED_IN)
	    switch(type) {
...
void *R_AllocStringBuffer(size_t blen, R_StringBuffer *buf)
{
    ...
    allocated_sb++;
    allocated_sb_elts += blen1;
    allocated_sb_size += blen;
