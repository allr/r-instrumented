...
- Dispatch
    ...
    This keyword provides two values. The first one counts how often
    the C function `DispatchOrEval` in the R interpreter has been
    called. The second one counts how often the final `return`
    statement has been reached plus how often the UseMethod R function
    failed with the error indicated by the message "no applicable
    method for x applied to an object of class y".
...
- Named
    ...
    This keyword lists four counter  values related to the *named*
    marker of an object within the R interpreter. This marker
    implements a simple version of reference counting, details are
    given in the R internals documentation. In the function that is
    used to set this marker, r-instrumented checks the old and new
    values of the marker. If both are the same, the fourth counter
    (keeped) is incremented. If they are different, the first counter
    (elements) is increased and a further check is made to see if the
    new value is lower (counted in the third counter, downgraded) or
    higher (second counter, promoted).

- ApplyDefine

    This keyword has two values which count how often a function has
    been used on the left-hand side of an assignment as a short-hand
    version of calling a function whose name ends in `<-`,
    e.g. `length(foo) <- 42`. This count ignores direct calls to such
    functions, e.g.

        `length<-`(foo, 42)

    The first value counts how often this short-hand call is used with
    the standard assignment operator (`<-` or `=`), the second value
    counts how often it is used with superassignment (`<<-`).

- DefineVar

    This keyword has two values which count how often a previously
    undefined object was defined - this includes new definitions after
    an old one has been removed via `rm`. The first value counts
    definitions via standard assignment (e.g. `<-` / `=` or `assign`),
    the second value counts definitions via superassignment
    (`<<-` or `assign` with `inherits=TRUE`). Changing the value of an
    existing object is counted in SetVar. This count may not cover all
    ways an object can be defined.

- SetVar

    This keyword has two values which count how often an
    already-existing object has been assigned a new value. The first
    value counts reassignments via standard assignment (e.g. `<-` or
    `=`), the second value counts reassignments via superassignment
    (`<<-` or `assign` with `inherits=TRUE`). This count does not
    cover all ways an object can receive a new value, for example the
    iteration variable of a `for` loop is not counted here.

    Note: This may be changed to count assignments that are currently
    ignored.

- DefineUserDb

    This keyword shows how many assignments were made into a special
    kind of environment that has the class `UserDefinedDatabase`. This
    is a special type of environment used for interfacing with native
    code and it cannot be created from a pure-R program.

    Note: Not tested

- ErrCountAssign

    From the code it appears that the value of this keyword counts how
    often an assignment was not completed successfully -- however, the
    author was unable to build an example that generates a non-zero
    value here.
...
- ErrorEvalSet
    ...
    This keyword has two values which count how often the evaluation
    of the RHS of an assignment failed. This is normally a fatal error
    that terminates an R program, but values larger than 1 are
    possible if the program catches such errors. The first value is
